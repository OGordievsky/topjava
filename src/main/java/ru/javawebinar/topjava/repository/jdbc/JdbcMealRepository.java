...
public class JdbcMealRepository implements MealRepository {
    ...
    public boolean delete(int id, int userId) {
        MapSqlParameterSource map = new MapSqlParameterSource()
                .addValue("user_id", userId)
                .addValue("meal_id", id);
        return namedParameterJdbcTemplate.update("DELETE FROM meals WHERE id=:meal_id AND user_id =:user_id", map) != 0;
    }
    ...
    public Meal get(int id, int userId) {
        MapSqlParameterSource map = new MapSqlParameterSource()
                .addValue("user_id", userId)
                .addValue("meal_id", id);
        List<Meal> meals = namedParameterJdbcTemplate.query("SELECT * FROM meals WHERE user_id=:user_id AND id=:meal_id", map, ROW_MAPPER);
    ...
    public List<Meal> getBetweenHalfOpen(LocalDateTime startDateTime, LocalDateTime endDateTime, int userId) {
        ...
        return namedParameterJdbcTemplate.query("SELECT * FROM meals WHERE user_id=:user_id " +
                "AND date_time >= :startDateTime AND date_time < :endDateTime ORDER BY date_time DESC", map, ROW_MAPPER);
    }
}
